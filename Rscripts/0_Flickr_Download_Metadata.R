...
# path_data = "~/Dropbox/KIT/FlickrEU/FlickrCNN/Seattle/"
...
max.try = 20
geturl.opts <- list(timeout = 20, maxredirs = max.try, verbose = T)
...
if (length(target.ids)==0) {
    is = NA 
    stop("nothing to go")    
} else {
    
    is = 1:length(target.ids)
}
...
final_res_msg = foreach (i = is, .errorhandling = "stop", .inorder = F, .verbose = F) %do% {
    ...
    api <- paste0("https://api.flickr.com/services/rest/?method=flickr.photos.search&format=json&api_key=", api_key, "&nojsoncallback=1&page=1&per_page=", max.perpage, "&bbox=", aoi.bbox.txt, "&min_taken_date=", mindate, "&max_taken_date=", maxdate, "&sort=", sort, "&privacy_filter=", "1", "&extras=", extras)
    ...
    if ( data.1st$photos$pages <=0 || length(data.1st$photos$photo) ==0) { # e.g., no photos with one (invalid) page
        ...
        if (npages > 1) {
            ...
            data.l.tmp[2:length(data.l.tmp)]  <- foreach (p.idx = 2:npages) %do% {
                ...
                cat(p.idx, ">")
                api.tmp <- paste0("https://api.flickr.com/services/rest/?method=flickr.photos.search&format=json&api_key=", api_key, "&nojsoncallback=1&page=", p.idx, "&per_page=", max.perpage, "&bbox=", aoi.bbox.txt, "&min_taken_date=", mindate, "&max_taken_date=", maxdate, "&sort=", sort, "&privacy_filter=", "1", "&extras=", extras)
        ...
        for (p.idx in 1:npages) { 
            ...
            res.l1 <- foreach (u = 1:nphotos.tmp, .errorhandling = "stop") %dopar% {
        ...
        metadata.tmp = metadata.tmp[match(unique(metadata.tmp$PhotoID), metadata.tmp$PhotoID),]
        ...
        owner_v = unique(as.character(metadata.tmp$Owner))
        ...
        userinfo_df = foreach (o_idx = 1:length(owner_v), .combine="rbind", .errorhandling = "stop") %dopar% {
            ...
            print(owner_tmp)
            api.tmp <- paste0("https://api.flickr.com/services/rest/?method=flickr.profile.getProfile&format=json&api_key=", api_key, "&nojsoncallback=1&user_id=", owner_tmp )
            ...
            Hometown = iconv(as.character(res.tmp$profile$hometown), from="UTF-8", to="ASCII", sub="")
            Hometown = ifelse(length(Hometown)==0, NA, Hometown)
            
            Country = iconv(as.character(res.tmp$profile$country), from="UTF-8", to="ASCII", sub="")
            Country = ifelse(length(Country)==0, NA, Country)
            
            City = iconv(as.character(res.tmp$profile$city), from="UTF-8", to="ASCII", sub="")
            City = ifelse(length(City)==0, NA, City)
            
            Occupation = iconv(as.character(res.tmp$profile$occupation), from="UTF-8", to="ASCII", sub="")
            Occupation = ifelse(length(Occupation)==0, NA, Occupation)
            
            api.tmp <- paste0("https://api.flickr.com/services/rest/?method=flickr.people.getInfo&format=json&api_key=", api_key, "&nojsoncallback=1&user_id=", owner_tmp )
            ...
            UserLocation = iconv(as.character(res.tmp$person$location), from="UTF-8", to="ASCII", sub="")
            UserLocation = ifelse(length(UserLocation)==0, NA, UserLocation)
            
            
            UserURL = as.character(res.tmp$person$profileurl)
        ...
        if (!is.null(dim(userinfo_df))) { 
            userinfo_df_expanded = userinfo_df[match(metadata.tmp$Owner, owner_v),]
        } else {
            userinfo_df_expanded = data.frame(matrix(nrow = nrow(metadata.tmp), data=NA, ncol=length(userinfo_df)))
            userinfo_df_expanded[match(metadata.tmp$Owner, owner_v),] = userinfo_df
            print(    userinfo_df)
            print(    userinfo_df_expanded)
            
        }
        
        metadata.tmp =   cbind(metadata.tmp, userinfo_df_expanded)
...
nrow(metadata.tmp)
length(unique(metadata.tmp$PhotoID))
...
# Retreiving the data (old code to delete)
...
for (i in 1:n.points) { 
    ...
    #     stop()
    # }
    nphotos_true = length(photo.ids.unique)
    print(nphotos_true)
    print(nphotos_true/nrow(aoi.dt.raw))
    nphotos.done.reduced.v[i] <- max(0, nphotos_true)
}

# i = 527 
# aoi.dt.raw[aoi.dt.raw$PhotoID == 32570535417,]  # photos duplicated


plot(nphotos.done.v, nphotos.done.reduced.v, xlab = "# of Flickr photos returned by API", ylab = "# of unique Flickr photos", main = "Large number of duplicated Flickr photos (Costa Rica, 2005-2019")
...
writeOGR(dt, dsn = "../Costa Rica_Data/FlickrCR_download/Aug2019_V1", layer = "FlickrC_AOI_Nphotos_unique.shp", driver = "ESRI Shapefile", overwrite_layer = T)
