...
# load(paste0(workdir, "Aug2019_V1/Flickr_CR_workspace_metadata_download_17Aug2019_2.RData"))
...
# costarica_natpark =  readOGR(paste0(path_data, "GIS data/NatParks_CR/"), layer = "NatParks_CR_Nov2019.gpkg", verbose = T)
...
path_originalphotos= "/DATA2TB/FlickrCR_download/Nov2019_V2_Photo/"
path_predictedphotos= "/DATA2TB/FlickrCR_Tagging_Nov2019/InceptionResnetV2_dropout30/FlickrCR_Photos_All/Result/InceptionResnetV2_dropout30/ClassifiedPhotos/"
path_predictedtags= "/DATA2TB/FlickrCR_Tagging_Nov2019/InceptionResnetV2_dropout30/FlickrCR_Photos_All/Result/InceptionResnetV2_dropout30/CSV/"


csv.done.names = list.files(path_predictedtags, pattern =  "\\.csv$")

# 299
# 
# aoi_idx = 327 # 299

which(str_detect(aois.done.newnames, "3850"))
...
for (aoi_idx in 1:length(aois.done.newnames)){ 
    ...
    csv_name = csv.done.names[str_detect(csv.done.names, pattern = paste0("(?<=CellID)", aoi_cellid))]
 
    csv_dt = read.csv(paste0(path_predictedtags, csv_name))
    
    aoi.dt[which(!(aoi.dt$PhotoID %in% csv_dt$PhotoID)),]
    
    csv_dt$PhotoID = str_extract(csv_dt$Filename, pattern = "(?<=photoid\\_)[0-9]*")
    
    aoi.dt_merged = merge(aoi.dt, csv_dt, by.all=PhotoID, all=T) # sometimes no tags in CSV? 
    
    warning(nrow(aoi.dt_merged) == nrow(aoi.dt))
    ...
    if (n_overlap > 0) { 
        print(paste0("CellID ", aoi_idx, "- ", n_overlap, " photo overlapped"))   
        ...
        for (np_idx in 1:length(natparks)) { 
            ...
            # 
            #     path_target = paste0(path_np_photos, "/OriginalPhotos")#,  p_dt$Year) # "/", p_filename_short)
            ...
            # predicted photos 
            predicted_path = list.files(path_predictedphotos, pattern = as.character(aoi_cellid ))
            stopifnot(length(predicted_path) == 1)  
            
            for (p_idx in 1:nrow(np_dt)) {
                p_dt = np_dt[p_idx,] 
                # print(p_idx)
                p_filename = list.files(paste0(path_predictedphotos, "/", predicted_path, "/", p_dt$Top1), pattern=p_dt$PhotoID, full.names = T)
                p_filename_short = list.files(paste0(path_predictedphotos, "/", predicted_path, "/", p_dt$Top1), pattern=p_dt$PhotoID, full.names = F)
                
                path_target = paste0(path_np_photos, "/",p_dt$Top1 )#,  p_dt$Year) # "/", p_filename_short)
                if (!dir.exists(path_target)) { 
                    dir.create(path_target, recursive = T)
                }
