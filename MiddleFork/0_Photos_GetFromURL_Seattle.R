...
library(tidyverse)
newphotos_df = read_csv("../UnlabelledData/Seattle/NewTestPhotos/mbsmtloop_flickrimages_20200213.csv")


str(newphotos_df)
library(RCurl)

path.expand(newphotos_df$path[1])
file(newphotos_df$path[1])


basename(newphotos_df$path[1])


checkImage = T 
library(imager)
registerDoMC(20)
foreach (f_idx = 1:nrow(newphotos_df)) %do% { 
    cat(f_idx)
    destfile = paste0("../UnlabelledData/Seattle/NewTestPhotos/Photos/", basename(newphotos_df$path[f_idx]))
    if (file.exists(destfile)) { 
        
        if (checkImage) { 
            img <- imager::load.image(destfile)
            ...
            if (!is.cimg(img)) {
                ...
                tryCatch(
                    download.file(newphotos_df$path[f_idx],destfile), 
                    finally = return()
                )            
            } else {
                 cat(".")
...
# writeOGR(aoi_spdf, dsn = "../DATA2TB_LINK/Flickr15Oct2019/", layer = "FlickrMiddlefork_predicted", driver="ESRI Shapefile")
...
for (t_idx in 1:3) { 
    ...
    pdf(paste0("../DATA2TB_LINK/Flickr15Oct2019/FlickrMiddlefork_Plot_Top", t_idx, ".pdf"), width = 20, height = 20)
